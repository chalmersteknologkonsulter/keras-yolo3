...
import argparse
'''
Don't touch anything - run prepare_test_set.py then predict.py then this
'''
def calcAndSavemAP(prefix,res_file):
  ...
  img_path = 'C:/Users/CTK-VR1/PycharmProjects/mAP/images'
  if os.path.exists(img_path):
    for dirpath, dirnames, files in os.walk(img_path):
      if str(dirpath) == 'images\\0' or str(dirpath) == 'images\\1' or str(dirpath) == 'images\\teade\\val2017':
        if not files:
  ...
  """
  tmp_files_path = "C:/Users/CTK-VR1/PycharmProjects/mAP/tmp_files"
  if not os.path.exists(tmp_files_path): # if it doesn't exist already
    os.makedirs(tmp_files_path)
    os.makedirs(tmp_files_path+'/0')
    os.makedirs(tmp_files_path+'/1')
    os.makedirs(tmp_files_path+'/teade')
    os.makedirs(tmp_files_path+'/teade/train2017')
    os.makedirs(tmp_files_path+'/augmented')

  results_files_path = prefix + res_file
  ...
  if show_animation:
    os.makedirs(results_files_path + "/images/correct")
    os.makedirs(results_files_path + "/images/incorrect")
  ...
  gt_dir = 'C:/Users/CTK-VR1/PycharmProjects/mAP/ground-truth/'
  ground_truth_files_list = []
  for subdir, dirs, files in os.walk(gt_dir):
    for f in files:
      ground_truth_files_list.append(os.path.join(subdir, f))
  ...
  for txt_file in ground_truth_files_list:
    ...
    #print(file_id.split("ground-truth/")[1])
    file_id = file_id.split('ground-truth/')[1]
    # check if there is a correspondent predicted objects file
    if not os.path.exists('C:/Users/CTK-VR1/PycharmProjects/mAP/predicted/' + file_id + ".txt"):
      error_msg = "Error. File not found: C:/Users/CTK-VR1/PycharmProjects/mAP/predicted/" +  file_id + ".txt\n"
  ...
  predicted_files_list = []
  for subdir, dirs, files in os.walk(pred_dir):
    for f in files:
      predicted_files_list.append(os.path.join(subdir, f))
  ...
  for class_index, class_name in enumerate(gt_classes):
    ...
    for txt_file in predicted_files_list:
      ...
      file_id = txt_file.split(".txt",1)[0]
      file_id = file_id.split('predicted/')[1]
      if class_index == 0:
        if not os.path.exists('C:/Users/CTK-VR1/PycharmProjects/mAP/ground-truth/' + file_id + ".txt"):
          error_msg = "Error. File not found: C:/Users/CTK-VR1/PycharmProjects/mAP/ground-truth/" +  file_id + ".txt\n"
          error_msg += "(You can avoid this error message by running extra/intersect-gt-and-pred.py)"
          error(error_msg)
    ...
    for class_index, class_name in enumerate(gt_classes):
      ...
      for idx, prediction in enumerate(predictions_data):
        ...
        if show_animation:
          ...
          try:
            filetype = '.jpg'
            ground_truth_img = img_path+'/'+file_id+filetype
            img = cv2.imread(img_path+'/'+file_id+filetype)
            img.shape[:2]
          except:
            filetype = '.png'
            ground_truth_img = img_path + '/' + file_id + filetype
        ...
        if show_animation:
          ...
          #cv2.waitKey(10) # show image for 10 ms
          # save image to results
          if status == "MATCH!":
            output_img_path = results_files_path + "/images/correct/" + class_name + "_prediction" + str(idx) + ".jpg"
          else:
            output_img_path = results_files_path + "/images/incorrect/" + class_name + "_prediction" + str(idx) + ".jpg"
      ...
      ap_per_class.append([class_name, ap])
      sum_AP += ap
  ...
  tmp_path = 'C:/Users/CTK-VR1/PycharmProjects/mAP/tmp_files/'
  ...
  return ap_per_class, mAP*100

if __name__=='__main__':
  annotations_file = 'test_anno.txt'  # File to evaluate mAP on
  result_prefix = 'C:/Users/CTK-VR1/PycharmProjects/mAP/results/'
  result_prefix += annotations_file.split('.txt')[0] + '/'
  model_name = '170_img_size_804'

  temp_ap_per_class, temp_mAP = calcAndSavemAP(result_prefix, model_name)